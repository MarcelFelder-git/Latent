...
Latent installs to the home screen and **runs offline**. That is not a bolted-on
extra — it falls out of the architecture. There is no backend to be cut off from:
the app is one HTML file, one bundle, and a GPU.

A hand-written service worker (~60 lines, no Workbox) splits the job by whether a
filename is unique. Everything under `/assets/` carries a content hash, so the same
name guarantees the same bytes — cache first. The HTML has a fixed name and is the
file that points at the current hashes, so it goes to the network first; serving it
from cache is exactly how you ship a stale build for a week.

Verified with the server switched off: the app boots, the UI renders, and the full
five-pass pipeline still draws the sample scene — a sunset column reading
`26,29,38` at the ground, `241,208,112` through the sun, `31,48,76` at the top of
the sky.

The 23 MB model runtime is deliberately **not** in the offline store. It is opt-in
and belongs to a feature you may never switch on; the browser's own HTTP cache
handles it.

The app icons are generated by a script rather than committed as opaque binaries:

```bash
python scripts/make-icons.py
```

The mark is the film strip the renderer already draws around the image — rounded
rectangles described analytically, supersampled 4× for smooth edges, and kept
inside the safe circle Android masks maskable icons to.
...
- **Installable:** web app manifest, generated icons, hand-written service worker
...
- **Offline covers the app, not the model.** Everything that runs on the GPU works
  without a network. The optional scene recognition needs its one-time download.
